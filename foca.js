...
client.on("message", (message) => {
  ...
  // if ((message.content.startsWith('!')) || (message.content > 300000 && message.content < 450000)){
  if ((message.content.startsWith('!'))){
    mensaje = message;
    textoMensaje = (message.content.replace('!', '')).toLowerCase();
  
  // if (textoMensaje > 300000 && textoMensaje < 450000) {
  //         let numIncidencia = textoMensaje;
  //         mensajeTemporal(mensaje.channel, numIncidencia, mensaje);
                   
  //       } else 
        if (['comandos','sql2008','sql2016','sql2019'].filter(e => e == textoMensaje).length > 0) {
          message.reply("", {
            embed: {
              author: `Foca BOT`,
              description: `El Fer mató al botsito, cualquier cosa [chiflen acá](http://wa.me/542616880732)`,
              color: 16774400,
            }});
        }
      }
